...
Since __visNetwork_0.1.2__ : select subset of nodes by the values of a column using ````selectedBy```` :
...
```{r}
# or new column
nodes$sample <- sample(c("sample 1", "sample 2"), 
...
## ````selectedBy```` & ````nodesIdSelection```` options

New features are available in __visNetwork_0.2.0__ : 
