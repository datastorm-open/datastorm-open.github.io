...
It's now possible to control the ````degree```` of depth (**fix/improve performance in 1.0.0**), and to enable this option also hovering nodes (``hover``, **>=1.0.0**). Using ``hover``, you can still use ``click`` to set a view :
...
New features are available in __visNetwork_0.2.0__ to customize ````selectedBy```` & ````nodesIdSelection```` : 
