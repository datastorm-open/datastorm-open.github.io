...
nodes <- data.frame(id = 1:3)
edges <- data.frame(from = c(1,2), to = c(1,3))
visNetwork(nodes = nodes, edges = edges, width = "100%", height = "300px") %>%
...
nodes <- data.frame(id = 1:10, 
                    ...
                    group = c("GrA", "GrB"),                                     # control groups on nodes 
                    value = 1:10,                                                # control size adding value
...
edges <- data.frame(from = sample(1:10,8), to = sample(1:10, 8),
                    label = paste("Edges", 1:8),                                # add labels on edges
...
```{r, echo = FALSE}
visNetwork(nodes, edges, width = "100%", height = "500px") %>% visOptions(highlightNearest = TRUE, nodesIdSelection = TRUE) %>%
  visPhysics(stabilization = FALSE)
```
...
```{r, echo = FALSE}
visNetwork(nodes, edges, width = "100%", height = "400px") %>% visEdges(arrow = "to") %>% 
...
```{r, echo = FALSE}
visNetwork(nodes, edges, width = "100%", height = "650px") %>% visOptions( highlightNearest = TRUE, nodesIdSelection = TRUE)  %>%
...
```{r, echo = FALSE}
visNetwork(nodes, edges, width = "100%", height = "300px") %>% visInteraction(hover = TRUE) %>%
