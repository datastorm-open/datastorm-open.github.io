...
As seen in introduction, *nodes* mut be a data.frame, with at least one column ```id```. You can add properties simply with more variables on data.frame. See **visNodes** for available options. 
...
```{r}
nodes <- data.frame(id = 1:4, shape = c("circle", "square"), 
                    label = LETTERS[1:4])
edges <- data.frame(from = c(2,4,3,3), to = c(1,2,4,2))
