...
The package proposes all the features available in **amcharts.js** API, and even more with special features for R : 

* easy to use
* custom shapes, styles, colors, sizes, ...
* works smooth on any modern browser for up to a few thousand nodes and edges
* interactivity controls (zoom, tooltip, events, ...)

## Prerequisite

It uses internally the package **pipeR** which is usefull to draw complex charts by keeping a certain level of readability in the code. 
Mainly the the functionnality used is `%>>%` (or `pipeline()`) but the package allow to do many more, see the official tutorial 
