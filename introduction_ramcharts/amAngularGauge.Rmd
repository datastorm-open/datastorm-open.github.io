...
amAngularGauge(x = 25) %>>% plot(width = "50%")
```

<h2 class="partdivtitle"> Add colored bands </h2>
```{r, echo = TRUE, eval = FALSE}
...
bands = data.frame(start = c(0, 40, 60), end = c(40, 60, 100), 
                   color = c("#00CC00", "#ffac29", "#ea3838"),
                   stringsAsFactors = FALSE)
amAngularGauge(x = 25, bands = bands) %>>% plot(width = "50%")
```

<h2 class="partdivtitle"> Add text </h2>
```{r, echo = TRUE, eval = FALSE}
amAngularGauge(x = 25, text = "km/h")
```
```{r, echo = FALSE, eval = TRUE}
amAngularGauge(x = 25, text = "km/h") %>>% plot(width = "50%")
```

<h2 class="partdivtitle"> Two axis </h2>
```{r, echo = TRUE, eval = FALSE}
...
bands2 = data.frame(start = c(100, 130, 170), end = c(130, 170, 200), 
                   ...
                   stringsAsFactors = FALSE)
amAngularGauge(x = 25, start = 0, end = 100, bands = bands,
               secondAxe = TRUE, start2 = 100, end2 = 200, bands2 = bands2)  %>>% plot(width = "50%")
```
...
```
```{r, echo = FALSE}
amAngularGauge(x = 25, export = TRUE)  %>>% plot(width = "50%")
```
...
```
```{r, echo = FALSE}
amAngularGauge(x = 25, export = TRUE, exportFormat = "JPG")  %>>% plot(width = "50%")
```
...
amAngularGauge(x = 25, 
       main = "My Pie",
        mainColor = "#00FF00", mainSize = 25, creditsPosition = "top-right")  %>>% plot(width = "50%")
```
...
```
```{r, echo = FALSE}
amAngularGauge(x = 25, theme = "patterns")  %>>% plot(width = "50%")
```
